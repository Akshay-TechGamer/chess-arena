...
import { leaderboardPage, type LeaderboardRow } from '@/lib/data/profilesRepo';
...
export default function LeaderboardPage() {
	...
	useEffect(() => {
		let cancelled = false;
		leaderboardPage(page, PAGE_SIZE)
			.then((result) => {
				if (!cancelled) {
					setRows(result.rows);
				...
				<>
					<div className="lb-panel">
						...
						{rows.map((row, index) => {
							...
							const rankClass = rank <= 3 ? RANK_CLASS[rank - 1] : '';
							return (
								<div key={row.id} className={`lb-row ${rankClass}${isYou ? ' lb-you' : ''}`}>
									...
									<span className="lb-player">
										...
										<span className="lb-name">
											{row.username}
											<span className="tier-chip">{tierFor(row.elo_rating)}</span>
											{isYou && <span className="you-tag">you</span>}
										</span>
									</span>
									<span
										className={`lb-num lb-rating${rank <= 3 || isYou ? ' lb-rating-hot' : ''}`}
									>
